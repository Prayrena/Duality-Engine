#include "Engine/Algorithm/Sorting.hpp"
...
void MergeSortAlgorithm::Merge(vector<int>& arr, int L, int M, int R)
{
...
int SmallSumAlgorithm::Merge(vector<int>& arr, int L, int M, int R)
{
	// create a new int vector the same size as original one
	vector<int>help;
	help.resize(R - L + 1);
